...
_RULES: tuple[tuple[str, str, re.Pattern[str], int | None], ...] = (
    ...
    ("stripe-secret-key", "high", re.compile(r"\b[rs]k_(?:live|test)_[A-Za-z0-9]{16,}\b"), 0),
    ("npm-token", "high", re.compile(r"\bnpm_[A-Za-z0-9]{36}\b"), 0),
    ("sendgrid-api-key", "high",
     re.compile(r"\bSG\.[A-Za-z0-9_-]{16,}\.[A-Za-z0-9_-]{16,}\b"), 0),
    ("azure-account-key", "high", re.compile(r"(?i)AccountKey=([A-Za-z0-9+/=]{40,})"), 1),
    ("openai-api-key", "medium",
     re.compile(r"\bsk-(?:proj-|svcacct-)?[A-Za-z0-9_-]{32,}\b"), 0),
    ("jwt", "medium",
